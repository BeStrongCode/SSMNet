...
class ChannelAttention(nn.Module):
    def __init__(self,in_channel,reduction=16):
        ...
        self.fc1 = nn.Conv2d(in_channel,in_channel // reduction, kernel_size=1, bias=False)
        self.relu = nn.ReLU()
        self.fc2 = nn.Conv2d(in_channel // reduction, in_channel, kernel_size=1, bias=False)
...
class MambaFusion(nn.Module):
    def __init__(self,range=2,ch=128,ch1=128,ch2=256,out=256):
        ...
        self.conv3 = nn.Conv2d(self.c_,out,1)
        self.mamba = ConvSSM1(self.c_, out)
    ...
    def forward(self,x1,x2):
        ...
        f = self.act(self.bn(self.att(x1, x2)))
        mamba = self.mamba(f)
        return  mamba
...
class FeatureFusion(nn.Module):
    def __init__(self,in_channel):
        ...
        self.weights = nn.Parameter(torch.ones(in_channel),requires_grad=True)
        self.epsilon = 0.0001
        self.mamba = ConvSSM1(in_channel,in_channel)
        torch.nn.init.normal_(self.weights, mean=1.0, std=0.01)
        self.conv1 = nn.Conv2d(in_channels=in_channel, out_channels=in_channel//2, kernel_size=1)
        self.finalconv = nn.Conv2d(in_channels=in_channel,out_channels=in_channel,kernel_size=3,stride=2,padding=1)
    def forward(self,x1,x2):
        weights = self.weights/(torch.sum(self.weights,dim=0) + self.epsilon)
        weights = weights.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        x2 = F.interpolate(x2, scale_factor=2, mode='bilinear', align_corners=False)
        x2 = self.conv1(x2)
        x3 = torch.cat([x1,x2],dim=1)
        x3 = weights * x3
        # out = self.finalconv(x3)
        # out = self.mamba(out)
        out = self.mamba(x3)
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.fusion = MambaFusion(2,ch0,ch0,ch[0],ch[0]) # mamba
